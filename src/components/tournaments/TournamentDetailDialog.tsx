...
export function TournamentDetailDialog({
  ...
  useEffect(() => {
    ...
    setLightbox(false);
  }, [tournament.id, tournament.image_url, tournament.community?.cover_image_url, open]);
  ...
  const cityLabel = [tournament.community?.address_city, tournament.community?.address_state]
    ...
    .join(" · ");
  const cover =
    !coverBroken && (tournament.image_url || tournament.community?.cover_image_url || null);
  const showActions = (canSignup && signupHref) || tournament.is_private || (canShare && onShare);
    ...
    <>
      ...
      >
        <div
          ...
          className="flex max-h-[min(92dvh,100%)] w-full max-w-lg flex-col overflow-hidden rounded-t-2xl bg-white shadow-xl dark:bg-[#0b1220] sm:rounded-2xl"
          onMouseDown={(e) => e.stopPropagation()}
        >
          <div className="relative aspect-[16/9] w-full shrink-0 overflow-hidden bg-[#0a1628]">
            {cover ? (
              <button
                type="button"
                onClick={() => setLightbox(true)}
                className="absolute inset-0 block h-full w-full"
                aria-label="Ampliar imagem do torneio"
              >
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-[#051024] to-[#2563eb]" />
            )}
            {tournament.is_private && (
              <span className="absolute left-3 top-3 z-[1] rounded-full bg-black/65 px-2.5 py-1 text-[11px] font-bold text-white">
                Só membros
              </span>
            )}
            <button
              type="button"
              onClick={onClose}
              className="absolute right-3 top-3 z-[1] rounded-full bg-black/55 px-3 py-1 text-xs font-semibold text-white hover:bg-black/70"
            >
              Fechar
            </button>
          </div>
          ...
          {showActions && (
            <div className="flex shrink-0 flex-wrap gap-2 border-t border-slate-200/80 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] dark:border-white/10 sm:px-5">
              ...
              {canShare && onShare && (
                <button
                  type="button"
                  onClick={onShare}
                  className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-[var(--toq-navy)] transition hover:border-[var(--toq-accent)] dark:border-white/15 dark:text-white sm:flex-none"
                >
                  Compartilhar
                </button>
              )}
      ...
      {lightbox && cover && (
        <div
          className="fixed inset-0 z-[95] flex items-center justify-center bg-black/90 p-3"
          role="presentation"
          onMouseDown={(e) => {
        ...
        >
          ...
          <img
            src={cover}
            alt={tournament.name}
            className="max-h-[min(92dvh,100%)] max-w-full rounded-lg object-contain"
          />
          <button
            type="button"
